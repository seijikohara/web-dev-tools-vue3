...
test.describe('Dashboard', () => {
  test('should display network and browser information', async ({ pageWithMocks }) => {
    ...
    await test.step('Verify page sections are visible', async () => {
      ...
      // Verify IP address and hostname from API mock
      await Promise.all([
        expect.soft(pageWithMocks.getByText('203.0.113.42')).toBeVisible(),
        expect.soft(pageWithMocks.getByText('test.example.com')).toBeVisible(),
      ])
    })
